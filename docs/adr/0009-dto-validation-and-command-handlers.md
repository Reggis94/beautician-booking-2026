# ADR 0009: DTO Validation and Command Handlers for Writes
...
## Decision
For write endpoints, controllers must:
- Map request input into a dedicated DTO that owns validation rules via Symfony Validator constraints.
- Create a command from the validated DTO and pass it to a command handler that executes the action.

## Consequences
- Validation rules live in DTOs instead of controllers.
- Controllers stay thin and consistent across domains.
- Handlers become the single entry point for write operations, easing reuse and testing.
